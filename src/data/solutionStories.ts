...
export const solutionStories: Record<string, SolutionStory> = {
  ...
  'grounded-agent-search': {
    ...
    capabilityHeading: { lead: 'Choose the depth.', emphasis: 'Keep the evidence.' },
    workflowHeading: {
      lead: 'From one agent method',
      emphasis: 'to the full retrieval stack.',
      body: 'The thin Pilot adapter maps structured agent calls to Cosift. The backend handles the crawl, index, retrieval, optional synthesis, and source-bearing response.',
    },
    closeHeading: { lead: 'Give one real question', emphasis: 'to Cosift.' },
    infographic: {
      kind: 'retrieval',
      eyebrow: 'Cosift product path',
      title: 'One backend from crawling to cited synthesis.',
      summary: 'Use the public corpus immediately or run the same open-source stack against sources and model endpoints under your control.',
      stages: [
        { label: 'Call', title: 'Choose a Cosift method', body: 'The agent selects search, contents, similar-document lookup, answer, or research.', items: ['help', 'method', 'JSON'] },
        { label: 'Retrieve', title: 'Run the configured path', body: 'BM25, dense HNSW, hybrid RRF, filters, reranking, and diversity controls compose per request.', items: ['BM25', 'HNSW', 'RRF'] },
        { label: 'Synthesize', title: 'Answer only when asked', body: 'Answer and research methods add model synthesis; plain retrieval remains available without it.', items: ['answer', 'research', 'limits'] },
        { label: 'Return', title: 'Keep sources and warnings', body: 'The structured result reports evidence, effective retrieval, fallbacks, and timing.', items: ['sources', 'warnings', 'took'] },
      ],
      artifact: {
        label: 'cosift.search',
        title: 'raft leader election',
        status: '3 results',
        summary: 'The agent receives ranked source records and the effective retriever, ready for inspection or a later synthesis step.',
        rows: [
          { key: 'retriever', value: 'bm25+dense:rrf', tone: 'accent' },
          { key: 'results', value: 'hits[] · URL + excerpt' },
          { key: 'warnings', value: '[]', tone: 'good' },
          { key: 'confidence', value: 'not fabricated' },
        ],
        footer: 'Representative response shape, not a benchmark',
      },
    },
  },
...
export const solutionFunnels: Record<string, SolutionFunnel> = {
  ...
  'grounded-agent-search': {
    eyebrow: 'Cosift workflow mapper',
    title: 'Bring one real question.',
    emphasis: 'Leave with the right Cosift call.',
    body: 'Tell us what the agent needs back. We will map it to search, contents, answer, or research and outline whether the public corpus or a self-hosted Cosift backend is the better fit.',
    choiceLabel: 'Desired result',
    choices: ['Ranked web results', 'One answer with citations', 'Multi-step research report', 'Similar documents', 'Clean contents for known URLs', 'Private self-hosted corpus'],
    contextLabel: 'What should the agent investigate or retrieve?',
    contextPlaceholder: 'Example: compare the latest approaches to durable agent memory and return the sources behind each tradeoff…',
    submitLabel: 'Map my Cosift workflow',
    deliverables: ['Recommended Cosift method', 'Example payload and response shape', 'Public or self-hosted deployment path'],
    success: 'Your Cosift workflow request is with the team.',
  },
  'web-access-for-agents': {
